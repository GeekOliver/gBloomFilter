...
func TestAddAndGet(t *testing.T) {
	filter := NewBloomFilter(1024, 3, false)
	// chaining operation
	...
	t.Logf("Hello exist:%t", filter.Check([]byte("Hello")))
	t.Logf("World exist:%t", filter.CheckString("World"))
	t.Logf("uint 16 exist:%t", filter.CheckUInt16(uint16(16)))
	t.Logf("uint 32 exist:%t", filter.CheckUInt32(uint32(32)))
	t.Logf("uint 64 exist:%t", filter.CheckUInt64(uint64(64)))

	t.Logf("key exist:%t", filter.Check([]byte("key")))
	t.Logf("exist exist:%t", filter.CheckString("exist"))
	t.Logf("uint 128 exist:%t", filter.CheckUInt16(uint16(128)))
	t.Logf("uint 33 exist:%t", filter.CheckUInt32(uint32(33)))
	t.Logf("uint 65 exist:%t", filter.CheckUInt64(uint64(65)))
...
func TestAddAndGet2(t *testing.T) {
	...
	stringLen := 30
	filter := NewBloomFilter(uint64(dataSize), 3, false)
...
func TestAddAndGetSync(t *testing.T) {
	...
	filter := NewBloomFilter(uint64(sizeData), 3, true)
	...
	fn := func(size int, wg *sync.WaitGroup) {
		...
		for i := 0; i < size; i++ {
			...
			if _, ok := m[randStr]; !ok {
				...
				// read
				exist := filter.CheckString(randStr)
				if !exist {
...
func BenchmarkFilter_Add(b *testing.B) {
	...
	filter := NewBloomFilter(uint64(dataTestSize), 3, false)
	b.ResetTimer()
	b.StartTimer()
